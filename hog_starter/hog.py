...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    total_points= 0 #keep track of theh total points#
    pig_out= False #check for the pig out rule#
    
    #roll dice num_rolls time#
    for i in range (num_rolls):
        outcome= dice()
        total_points+= outcome #add the outcome to the total point
        
        #check for pig out if any outcome is 1#
        if outcome==1:
            pig_out= True
   #if pig out true, return 1 otherwise return the total points#
    return 1 if pig_out else total_points
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    #check the free bacon rule #
    if num_rolls==0:
        free_bacon= 1+max(opponent_score//10, opponent_score%10)
        return free_bacon;
    else:
    #call roll_dice function to get the turn score#
        turn_score= roll_dice(num_rolls,dice)
    #returnn the max of free bacon rule and the turn score #
        return turn_score;
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score<goal and opponent_score<goal:
        if who==0:
            pointstotal=take_turn(strategy0(score,opponent_score),opponent_score, select_dice(score, opponent_score))
        else:
            pointstotal=take_turn(strategy1(opponent_score,score),score, select_dice( opponent_score,score))
        if who==0:
            score+=pointstotal
        else:
            opponent_score+=pointstotal
            
#check for the swine swap rule
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for num_rolls in range(1, 11):
        averaged_roll = make_averaged(roll_dice, 1000)  # You can adjust the number of samples as needed.
...
def swap_strategy(score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    #benefitial swap
    if score + max(opponent_score // 10, opponent_score % 10) + 1==opponent_score/2:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    "*** YOUR CODE HERE ***"
    # Determine whether to use four-sided or six-sided dice
    dice = select_dice(score, opponent_score)
    
    # Call the Bacon Strategy
    bacon_roll = bacon_strategy(score, opponent_score)
    
    # Call the Swap Strategy
    swap_roll = swap_strategy(score, opponent_score)
    
    if bacon_roll == 0:
        return 0
    elif swap_roll == BASELINE_NUM_ROLLS:
        return BASELINE_NUM_ROLLS
    else:
        return BASELINE_NUM_ROLLS  # Fallback to a default strategy
